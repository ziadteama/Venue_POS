...
try {
  ...
  $nodeMsi = ""
  $provFile = Join-Path $InstallRoot "deployment\provision.env"
  if (Test-Path $provFile) {
    Get-Content $provFile | ForEach-Object {
      if ($_ -match '^\s*NODE20_MSI_PATH\s*=\s*(.+)\s*$') { $nodeMsi = $Matches[1].Trim() }
    }
  }
  if ($nodeMsi) {
    Ensure-VenuePosNode20 -MsiPath $nodeMsi | Out-Null
  } else {
  ...
  $nodePath = Resolve-VenuePosNode20Exe -NodeExe $NodeExe
  ...
  if ($SkipNativeRebuild) {
    ...
    Write-Host "==> Skipping native rebuild (-SkipNativeRebuild)"

  } else {

    Invoke-VenuePosNativeRebuild -InstallRoot $InstallRoot
  ...
  $pm2Home = Install-VenuePosPm2Agent -InstallRoot $InstallRoot -AgentRoot $agentRoot -NodeExe $nodePath -SkipStartup:$SkipPm2Startup
