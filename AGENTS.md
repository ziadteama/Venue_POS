...
**Web dashboard = `hub_owner` + `hub_manager` only** (`DASHBOARD_ROLES`). Enforced in `loginManager`, dashboard `ProtectedRoute`, and `canAccessDashboardPath` in `@venue-pos/shared`.
...
|------|-----------------|----------------|
| **hub_owner** (business) | Overview, analytics, cheques (read), orders, shifts/EOD, approvals, activity, health | No |
| **hub_manager** (ops) | Menus, staff, settings, activity, health | Policy PIN `9999` where API accepts hub |
...
| Page | Hub owner | Hub manager |
|------|-----------|-------------|
| Overview / Analytics | Yes | No |
| Cheques / Shifts / EOD / Orders / Approvals | Yes | No |
| Menus / Staff / Settings | No | Yes |
| Activity / Health | Yes (read) | Yes |

Path guards: `packages/shared/src/hub-access.js`. Login redirect: owner → `/`, manager → `/menus`.
...
**Phase 5 in progress** (`Phase-5` branch) — analytics, venue config, shifts + EOD, staff management (hub), system health, full audit log, **hub_owner / hub_manager RBAC split**. Inventory out of scope. See `docs/TEAM_LOG.md`.
...
| Refund approval / force refund | — | `hub_owner` (or hub staff per API) | Dashboard `/approvals` |
| Shift over/short | Cashier | Manager PIN | POS close shift |
| Staff CRUD | `hub_manager` | — | Dashboard `/users` |
| EOD / revenue review | `hub_owner` | — | Dashboard `/shifts` |
