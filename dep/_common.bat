...
if not exist "%WIN_OPS%\install.ps1" (
  ...
  echo.
  echo These scripts install from a local bundle — they do not download from the internet.
  echo.
  echo 1. Download venue-pos-till-windows-*.zip from GitHub Actions
  echo    (workflow: Build till bundle ^(Linux + Windows^) — artifact: venue-pos-till-windows^)
  echo 2. Extract the zip to C:\Venue_POS
  echo 3. Run deployment\install-all.bat as Administrator
  echo.
  echo Expected file: %WIN_OPS%\install.ps1
